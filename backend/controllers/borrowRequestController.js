...
const { saveBorrowRequestPdf, generateBorrowRequestPdf } = require('../utils/pdfGenerator');
  ...
  stockReconcilePromise = (async () => {
    try {
      const requests = await BorrowRequest.find({
        status: { $in: ['pending_head', 'head_approved', 'approved', 'ready_pickup', 'borrowed', 'returned', 'rejected'] }
      }).select('_id equipment quantity status stock_reserved released_at');

      for (const request of requests) {
        if (['pending_head', 'head_approved', 'approved', 'ready_pickup'].includes(request.status) && !request.stock_reserved) {
...
exports.headAction = async (req, res, next) => {
  try {
    ...
    if (action === 'reject') {
      emitEquipmentUpdate({ action: 'updated', equipmentId: String(request.equipment._id) });
    }
...
exports.prepareEquipment = async (req, res, next) => {
  try {
    ...
    if (!['head_approved', 'approved'].includes(request.status)) {
      return res.status(400).json({
        success: false,
    ...
    emitEquipmentUpdate({ action: 'updated', equipmentId: String(request.equipment) });
    await notifyStudentBorrowStatus(request);
    notifyUserRefresh(String(request.student), {
      type: 'borrow_request_status_changed',
      requestId: String(request._id),
...
exports.rejectBorrowRequest = async (req, res, next) => {
  try {
    ...
    if (!['pending', 'pending_lecturer', 'pending_head', 'approved'].includes(request.status)) {
      return res.status(400).json({
        success: false,
...
// @desc    Mark replacement as completed (for Lost/Damaged-replace returns)
// @route   PUT /api/borrow-requests/:id/complete-replacement
// @access  Private (Lab Assistant)
exports.completeReplacement = async (req, res, next) => {
  try {
    ...
    if (request.status !== 'returned') {
      return res.status(400).json({ success: false, message: 'Request has not been returned yet' });
    }

    if (!request.student_will_replace) {
      return res.status(400).json({ success: false, message: 'No replacement was required for this request' });
    }

    if (request.replacement_completed) {
      return res.status(400).json({ success: false, message: 'Replacement has already been verified' });
    }

    // Lost items: available was withheld at return time — restore it now
    ...
    request.replacement_completed = true;
    request.replacement_completed_at = new Date();
    request.replacement_verified_by = req.user.id;
    if (notes && notes.trim()) request.replacement_notes = notes.trim();
    await request.save();
    ...
    await logAuditEvent({
      ...
      userId: req.user.id,
      userEmail: req.user.email,
      actionType: 'replacement_completed',
      entityType: 'borrow_request',
      entityId: request._id,
      status: 'success',
      details: {
