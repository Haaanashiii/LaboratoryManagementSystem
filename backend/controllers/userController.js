...
const ALLOWED_ROLES   = ['admin', 'lecturer', 'head_of_lab', 'lab_assistant', 'student'];
...
exports.updateUser = async (req, res, next) => {
  try {
    const { name, role, department, studentId, phone, status, password, email: newEmail } = req.body;
    ...
    // Handle email change (admin only)
    if (newEmail && req.user.role === 'admin') {
      const parsedEmail = parseEmail(newEmail);
      if (!parsedEmail.isValid) {
        return res.status(400).json({ success: false, message: 'Please provide a valid email' });
      }
      if (!isAllowedRegistrationDomain(parsedEmail.domain)) {
        return res.status(403).json({ success: false, message: 'Email domain is not allowed' });
      }
      const targetRole = role || user.role;
      if (!isDevBypassEmail(parsedEmail.normalizedEmail) && !isAllowedDomainForRole(targetRole, parsedEmail.domain)) {
        return res.status(403).json({ success: false, message: 'Email domain is not allowed for this role' });
      }
      if (parsedEmail.normalizedEmail !== user.email) {
        const emailTaken = await User.findOne({ email: parsedEmail.normalizedEmail });
        if (emailTaken) {
          return res.status(400).json({ success: false, message: 'Email is already in use by another account' });
        }
        user.email = parsedEmail.normalizedEmail;
      }
    }
