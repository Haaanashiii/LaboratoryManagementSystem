...
export default function HeadFinalApprovalSkeleton() {
  return (
    <div className="w-full space-y-4 px-2 py-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
        <div className="flex items-center gap-4">
          <Shimmer className="h-12 w-12 rounded-xl flex-shrink-0 bg-green-100" />
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {[1, 2, 3].map((i) => <Shimmer key={i} className="h-8 w-28 rounded-lg" />)}
        <Shimmer className="h-9 flex-1 min-w-[160px] rounded-xl ml-auto" />
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              {['#', 'Equipment', 'Qty', 'Status', 'Forwarded', 'Action'].map((h, i) => (
