...
export function DropdownMenuContent({ children, align = 'start', className = '', style, ...props }) {
  ...
  // Clamp dropdown within viewport after it paints
  useLayoutEffect(() => {
  ...
  return (
    <div
      ref={contentRef}
      className={`absolute ${alignmentClasses[align]} mt-2 z-50 min-w-[8rem] max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-md border p-1 shadow-md animate-in fade-in-0 zoom-in-95 ${defaultBase} ${className}`}
      style={style}
      {...props}
    >
      {children}
