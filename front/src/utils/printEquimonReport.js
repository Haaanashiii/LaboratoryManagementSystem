...
import auditLogoUrl from '@/assets/images/Tower2.png';
...
const PDF_STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── KPI cards ── */
  .kpis { display: grid; grid-template-columns: repeat(4,1fr); border: 1px solid var(--rule); background: linear-gradient(180deg,#f8fafc 0%,var(--paper) 100%); }
  ...
  table.r tbody tr.group-row td { background: var(--paper-2) !important; font-family: var(--serif); font-weight: 600; color: var(--navy); font-size: 10.5px; padding: 6px 13px; border-bottom: 1px solid var(--blue); border-top: 0.5px solid var(--rule); }
  table.r tbody tr.group-row td .grp-count { font-family: var(--mono); font-size: 8px; color: var(--blue-2); margin-left: 10px; letter-spacing: 0.1em; font-weight: 500; }
  ...
  .d-bar > span::after { content: ""; position: absolute; right: -1px; top: 0; bottom: 0; width: 2px; background: var(--blue); }
  .d-val { font-family: var(--mono); font-size: 9.5px; color: var(--navy); text-align: right; font-weight: 600; }
  /* ── Note ── */
  .note { font-family: var(--serif); font-style: italic; font-size: 10.5px; color: var(--ink-2); padding: 9px 13px; border-left: 2px solid var(--blue); background: rgba(37,99,235,0.05); margin: 12px 0; }
...
function letterheadHtml(logoSrc) {
  ...
  <header class="letterhead">
    <img src="${src}" alt="ITS Logo" class="crest" />
    <div>
...
export function printFullReleaseReport(records, summary, { startDate, endDate, lecturerFilter } = {}, admin = {}) {
  ...
  const tableRows = records.map(r => {
    ...
    const cond = r.returnCondition;
    const condCls = cond === 'Good' ? 'b-ok' : cond === 'Damaged' ? 'b-warn' : cond === 'Lost' ? 'b-bad' : 'b-mute';
    ...
    return `<tr>
      ...
      <td>${st === 'returned' ? `<span class="badge ${condCls}">${condLabel}</span>` : '<span class="badge b-mute">—</span>'}</td>
    </tr>`;
  }).join('');
  ...
  // Return condition summary — only for records that have been returned
  const returnedRecords = records.filter(r => (r.status || '').toLowerCase() === 'returned');
  const conditionCounts = { Good: 0, Damaged: 0, Lost: 0 };
  returnedRecords.forEach(r => {
    const c = r.returnCondition;
    if (c === 'Good' || c === 'Damaged' || c === 'Lost') conditionCounts[c] += 1;
    else conditionCounts.Good += 1; // treat no condition as Good
  });
  const returnedTotal = returnedRecords.length;
  const maxCondCount  = Math.max(1, ...Object.values(conditionCounts));

  const conditionRows = [
    { label: 'Good',    cls: 'b-ok',   barColor: 'var(--ok)',   count: conditionCounts.Good },
    { label: 'Damaged', cls: 'b-warn',  barColor: 'var(--warn)', count: conditionCounts.Damaged },
    { label: 'Lost',    cls: 'b-bad',   barColor: 'var(--bad)',  count: conditionCounts.Lost },
  ].map(({ label, cls, barColor, count }) => {
    const pct = returnedTotal ? Math.round((count / returnedTotal) * 100) : 0;
    return `<div class="dist-row" style="grid-template-columns:70px 1fr 90px;align-items:center">
      <div><span class="badge ${cls}">${label}</span></div>
      <div class="d-bar"><span style="width:${(count / maxCondCount) * 100}%;background:${barColor}"></span></div>
      <div class="d-val" style="font-size:9.5px">${count} item${count !== 1 ? 's' : ''} &nbsp;·&nbsp; ${pct}%</div>
    </div>`;
  }).join('');

  const returnedDetailRows = returnedRecords.map(r => {
    const cond = r.returnCondition || 'Good';
    const condCls = cond === 'Good' ? 'b-ok' : cond === 'Damaged' ? 'b-warn' : 'b-bad';
    const timing = r.returnTiming || '—';
    const timingCls = timing === 'late' ? 'b-bad' : timing === 'early' ? 'b-ok' : 'b-mute';
    return `<tr>
      <td class="mono">${fmtDate(r.actualReturnDate || r.returnDate)}</td>
      <td>${r.studentName || '—'}</td>
      <td>${r.equipmentName || '—'}</td>
      <td class="num">${r.quantity || '—'}</td>
      <td><span class="badge ${condCls}">${cond}</span></td>
      <td><span class="badge ${timingCls}">${timing.replace(/_/g, ' ')}</span></td>
      <td style="font-size:9.5px;color:var(--ink-2);max-width:120px;white-space:normal">${r.returnRemarks || '—'}</td>
    </tr>`;
  }).join('');
  ...
  const body = pageHtml(`
    ...
    <div class="title-block">
      ...
      <p class="title-deck">A complete record of every equipment release authorised under lecturer custody during the reporting period, including borrower, quantity, lifecycle status, and return condition.</p>
    </div>
    ${metaHtml([
    ...
    <div class="section" style="margin-top:20px">
      ${sectionHtml('04', 'Return Condition Summary', `${returnedTotal} returned`)}
      ${returnedTotal === 0
        ? `<div class="note">No items have been returned in this period yet.</div>`
        : `<div class="two-col" style="gap:20px">
            <div>
              <div class="kpis" style="grid-template-columns:repeat(3,1fr)">
                <div class="kpi"><div class="k-label">Good</div><div class="k-val" style="color:var(--ok)">${conditionCounts.Good}</div><div class="k-sub">in good condition</div></div>
                <div class="kpi"><div class="k-label">Damaged</div><div class="k-val" style="color:var(--warn)">${conditionCounts.Damaged}</div><div class="k-sub">returned with damage</div></div>
                <div class="kpi" style="border-right:none"><div class="k-label">Lost</div><div class="k-val" style="color:var(--bad)">${conditionCounts.Lost}</div><div class="k-sub">reported as lost</div></div>
              </div>
              <div class="dist" style="margin-top:12px">${conditionRows}</div>
            </div>
            <div>
              <table class="r" style="font-size:10px">
                <thead><tr>
                  <th style="width:75px">Return Date</th>
                  <th>Borrower</th>
                  <th>Equipment</th>
                  <th class="num" style="width:36px">Qty</th>
                  <th style="width:72px">Condition</th>
                  <th style="width:60px">Timing</th>
                  <th>Remarks</th>
                </tr></thead>
                <tbody>${returnedDetailRows}</tbody>
              </table>
            </div>
          </div>`
      }
    ...
    <div class="section">
      ${sectionHtml('05', 'Detailed Release Ledger', `${records.length} entries`)}
      <table class="r">
        <thead><tr>
        ...
        <tbody>
          ...
          <tr class="total-row">
            ...
            <td class="sc">${returnedTotal} returned</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export function printWeeklySummaryReport(weeklySummary, summary, { startDate, endDate } = {}, admin = {}) {
  ...
  const weekRows = weeklySummary.map(wk => {
    ...
    return `
      ...
      <tr style="background:rgba(219,234,254,0.35)">
...
const AUDIT_OVERRIDES = `
  ...
  .page { background: #fff; }
  .kpis { background: #fff; }
  table.r tbody tr:nth-child(even) td { background: rgba(0,0,0,0.025); }
`;

const AUDIT_NORMAL = new Set(['login_success', 'login_failed', 'borrow_created', 'borrow_released', 'borrow_returned']);
...
function auditLetterheadHtml() {
  const src = auditLogoUrl.startsWith('data:') || /^https?:\/\//.test(auditLogoUrl)
    ? auditLogoUrl
    : `${window.location.origin}${auditLogoUrl}`;
  return `
  <header class="letterhead">
    <img src="${src}" alt="ITS Logo" class="crest" />
    <div>
...
const AUDIT_DISPLAY_ROWS = 30;

export function printAuditTrailReport(logs = [], { startDate, endDate } = {}, admin = {}) {
  const period = `${startDate || 'Start'} – ${endDate || 'Today'}`;
  const docId  = `EQM-AUD-${dateTag().replace(/-/g, '')}-A`;
  const now    = genTimestamp();
  const total  = logs.length;

  const successCount   = logs.filter(l => l.action_type === 'login_success').length;
  const failedCount    = logs.filter(l => l.action_type === 'login_failed').length;
  const sensitiveCount = logs.filter(l => !AUDIT_NORMAL.has(l.action_type)).length;
  const uniqueUsers    = new Set(logs.map(l => l.user?.email || l.user_email).filter(Boolean)).size;

  // Actions by type
  const actionMap = {};
  logs.forEach(l => { const k = l.action_type || 'unknown'; actionMap[k] = (actionMap[k] || 0) + 1; });
  const actionEntries = Object.entries(actionMap).sort((a, b) => b[1] - a[1]);
  const maxAction = Math.max(1, ...actionEntries.map(([, v]) => v));

  // Activity by role
  const roleMap = {};
  logs.forEach(l => { const r = l.user?.role || l.user_role || 'unknown'; roleMap[r] = (roleMap[r] || 0) + 1; });
  const roleEntries = Object.entries(roleMap).sort((a, b) => b[1] - a[1]);
  const maxRole = Math.max(1, ...roleEntries.map(([, v]) => v));

  const actionBars = actionEntries.slice(0, 6).map(([name, cnt]) => {
    const pct = (cnt / maxAction) * 100;
    const isBad = !AUDIT_NORMAL.has(name);
    return `<div class="dist-row" style="grid-template-columns:130px 1fr 52px">
      <div class="d-label" style="${isBad ? 'color:var(--bad);' : ''}">${name.replace(/_/g, ' ')}</div>
      <div class="d-bar"><span style="width:${pct}%;${isBad ? 'background:var(--bad)' : ''}"></span></div>
      <div class="d-val">${cnt} · ${total ? Math.round((cnt / total) * 100) : 0}%</div>
    </div>`;
  }).join('') || '<div class="sc">No data</div>';

  const roleBars = roleEntries.map(([name, cnt]) => {
    const pct = (cnt / maxRole) * 100;
    return `<div class="dist-row" style="grid-template-columns:90px 1fr 52px">
      <div class="d-label" style="text-transform:uppercase;font-size:8.5px;letter-spacing:0.08em">${name.replace(/_/g, ' ')}</div>
      <div class="d-bar"><span style="width:${pct}%"></span></div>
      <div class="d-val">${cnt} · ${total ? Math.round((cnt / total) * 100) : 0}%</div>
    </div>`;
  }).join('') || '<div class="sc">No data</div>';

  const tableRows = logs.slice(0, AUDIT_DISPLAY_ROWS).map(l => {
    const d = new Date(l.createdAt);
    const time   = isNaN(d.getTime()) ? '—' : d.toLocaleTimeString('en-US', { hour12: false });
    const email  = l.user?.email  || l.user_email  || '—';
    const role   = (l.user?.role  || l.user_role   || '—').replace(/_/g, ' ');
    const action = l.action_type  || '—';
    const ip     = _auditIp(l.ip_address);
    const isOk   = l.status === 'success';
    return `<tr>
      <td class="mono" style="font-size:9px;white-space:nowrap">${time}</td>
      <td style="font-size:9.5px;color:var(--ink-2)">${email}</td>
      <td style="font-size:9.5px;text-transform:capitalize">${role}</td>
      <td><span class="badge b-mute" style="font-size:7px">${action}</span></td>
      <td class="mono" style="font-size:9px">${ip}</td>
      <td><span class="badge ${isOk ? 'b-ok' : 'b-bad'}">${isOk ? 'ok' : 'fail'}</span></td>
    </tr>`;
  }).join('');

  const moreNote = total > AUDIT_DISPLAY_ROWS
    ? `<div style="padding:8px 13px;font-family:var(--mono);font-size:8px;letter-spacing:0.1em;text-transform:uppercase;color:var(--mute);border-top:0.5px dashed var(--rule)">
        Continue on next page — ${total - AUDIT_DISPLAY_ROWS} more entries · export full CSV for complete dataset
      </div>`
    : '';
  ...
  const body = `<div class="page">
    ${auditLetterheadHtml()}
    <div class="title-block">
      <div class="title-eyebrow">System &amp; Security · Full Disclosure</div>
      <h1 class="title-h1">Audit Trail Report</h1>
      <p class="title-deck">A complete record of every action recorded by the Equimon platform during the reporting period — including authentications, inventory mutations, and account changes.</p>
    </div>
    ${metaHtml([
      { label: 'Document ID', value: docId, mono: true },
      { label: 'Period',      value: period },
      { label: 'Records',     value: `${total} entries` },
      { label: 'Issued by',   value: admin?.email || 'admin@its.ac.id' },
    ])}
    <div class="section" style="margin-top:4px">
      ${sectionHtml('01', 'At-a-glance', 'Period totals')}
      ${kpiHtml([
        { label: 'Total',           value: total,          sub: 'across the period' },
        { label: 'Successful',      value: successCount,   sub: `events across ${uniqueUsers} users` },
        { label: 'Active Attempts', value: failedCount,    sub: 'events · investigation suggested' },
        { label: 'Sensitive',       value: sensitiveCount, sub: 'admin, inv, head, stud, etc.' },
      ])}
    </div>
    <div class="two-col" style="margin-top:20px">
      <div class="section" style="margin-top:0">
        ${sectionHtml('02', 'Actions by Type')}
        <div class="dist">${actionBars}</div>
        <div class="sc" style="margin-top:7px">One-side share of total entries: <strong style="color:var(--bad)">RED</strong> = sensitive (FAIL)</div>
      </div>
      <div class="section" style="margin-top:0">
        ${sectionHtml('03', 'Activity by Role')}
        <div class="dist">${roleBars}</div>
      </div>
    </div>
    <div class="section">
      ${sectionHtml('04', 'Detailed Audit Trail', `${Math.min(total, AUDIT_DISPLAY_ROWS)} of ${total} entries · pg 1`)}
      <table class="r">
        <thead><tr>
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"/>
    <title>Audit Trail Report — Equimon</title>
    <style>${PDF_STYLES}${AUDIT_OVERRIDES}</style>
  </head><body>${body}</body></html>`);
  w.document.close();
  setTimeout(() => w.print(), 600);
}
