...
const PORTAL_ORIGIN_KEY = 'authPortalOrigin';
const STORAGE_MODES = {
  LOCAL: 'local',
  SESSION: 'session',
};
...
const storeAuthData = ({ token, userData, rememberMe = true, portalOrigin = 'student' }) => {
  ...
  otherStorage.removeItem(PORTAL_ORIGIN_KEY);

  targetStorage.setItem(TOKEN_KEY, token);
  targetStorage.setItem(USER_KEY, JSON.stringify(userData));
  targetStorage.setItem(PORTAL_ORIGIN_KEY, portalOrigin);
...
const clearAuthStorage = () => {
  ...
  localStorage.removeItem(PORTAL_ORIGIN_KEY);
  sessionStorage.removeItem(TOKEN_KEY);
  sessionStorage.removeItem(USER_KEY);
  sessionStorage.removeItem(PORTAL_ORIGIN_KEY);
};
...
export const getPortalOrigin = () => {
  const storage = getActiveAuthStorage();
  return storage.getItem(PORTAL_ORIGIN_KEY);
};
...
const resolveApiAssetUrl = (value) => {
  ...
  // Strip internal Docker/localhost origins — the browser can't reach them.
  // Extract just the path so nginx proxies it correctly.
  const internalOrigin = normalizedValue.match(/^https?:\/\/(?:localhost|backend)(?::\d+)?(\/.*)/i);
  if (internalOrigin) return internalOrigin[1];
...
export const api = {
  ...
  auth: {
    ...
    adminLogin: async (email, password, rememberMe = false) => {
      ...
      const { token, ...userData } = data.data;
      storeAuthData({ token, userData, rememberMe, portalOrigin: 'admin' });
  ...
  entities: {
    ...
    Equipment: {
      ...
      uploadImage: async (file) => {
        ...
        return resolveApiAssetUrl(data.data.path || data.data.url || data.data.filename);
      },
      uploadImageFromUrl: async (imageUrl) => {
        const data = await request('/equipment/upload-image-from-url', {
          method: 'POST',
          body: JSON.stringify({ url: imageUrl }),
        });
        return resolveApiAssetUrl(data.data.path || data.data.url);
      },
    },
    ...
    BorrowRequest: {
      list: async () => {
        ...
        return data.data.map(item => {
          const eqImages = item.equipment && typeof item.equipment === 'object'
            ? normalizeEquipmentImages(item.equipment)
            : { image_url: '', images_urls: [] };
          const equipment_image_url = eqImages.image_url
            || resolveApiAssetUrl(item.equipment_image_url)
            || '';
          return { ...item, id: item._id || item.id, equipment_image_url, equipment_images_urls: eqImages.images_urls };
        });
      },
      myRequests: async () => {
      ...
      completeReplacement: async (id, notes = '') => {
        const data = await request(`/borrow-requests/${id}/complete-replacement`, {
          method: 'PUT',
