...
export default function LoginPage() {
  ...
  const handleSignupChange = (e) => {
    const { name, value } = e.target;
    const lettersOnly = ['firstName', 'lastName', 'middleName', 'department'];
    const sanitized = lettersOnly.includes(name)
      ? value.replace(/[^a-zA-Z\s''-]/g, '')
      : value;
    setSignupForm(prev => ({ ...prev, [name]: sanitized }));
  ...
  const isGibberish = (str, { loose = false } = {}) => {
    const words = str.trim().split(/\s+/).filter(Boolean);
    for (const word of words) {
      if (word.length < 2) return true;
      if (/(.)\1{2,}/.test(word)) return true;
      const vowelThreshold = loose ? 5 : 3;
      if (word.length >= vowelThreshold && !/[aeiouAEIOU]/.test(word)) return true;
    }
    return false;
  };
  ...
  const validateSignup = () => {
    ...
    else if (isGibberish(signupForm.lastName)) newErrors.lastName = 'Please enter a valid last name.';
    ...
    else if (isGibberish(signupForm.firstName)) newErrors.firstName = 'Please enter a valid first name.';

    if (signupForm.middleName.trim()) {
      if (!namePattern.test(signupForm.middleName.trim())) newErrors.middleName = t('errorNameInvalid');
      else if (isGibberish(signupForm.middleName)) newErrors.middleName = 'Please enter a valid middle name.';
    }
    ...
    else if (isGibberish(signupForm.department, { loose: true })) newErrors.department = 'Please enter a valid department.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const normalizedRole = (user?.role || '').toLowerCase();
      const isStaffRole = ADMIN_ACCESS_ROLES.includes(normalizedRole);

      if (location.state?.from?.pathname) {
        const from = location.state.from.pathname;
        const isAdminOnlyPath = ['/users', '/admin-dashboard', '/admin-audit-logs', '/admin-settings', '/reports', '/borrow-reports', '/admin-equipment-process'].some(
          p => from === p || from.startsWith(p + '/')
        );
        if (!isAdminOnlyPath || isStaffRole) {
          navigateWithTransition(from);
          return;
        }
      }

      const defaultDestination = isStaffRole ? '/admin-dashboard' : '/dashboard';
  ...
  const handleSignupSubmit = async (e) => {
    ...
    const fullName = [signupForm.firstName.trim(), signupForm.middleName.trim(), signupForm.lastName.trim()]
      .filter(Boolean).join(' ');
  ...
  const featureItems = [
    { icon: ClipboardList, label: 'Browse & Submit', desc: 'Request lab equipment in seconds from the full catalog' },
    { icon: Bell, label: 'Live Updates', desc: 'Track approvals, readiness, and returns in real time' },
    { icon: RotateCcw, label: 'Returns Synced', desc: 'Conditions verified and inventory updated automatically' },
  ];
    ...
    <div className="min-h-screen flex relative overflow-hidden" style={{ background: '#0A0A0F' }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative z-10 items-center justify-center p-16">
        <div className="max-w-[420px] w-full">
          ...
          {/* Logo */}
          <div className="mb-14 lp-a1">
            <img src={itsLogo} alt="ITS Logo" className="h-28 w-auto object-contain" style={{ mixBlendMode: 'multiply', filter: 'brightness(0) invert(1)' }} />
          </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 relative z-10">
        ...
        >
          ...
          <div className="mb-8 lp-a1">
            <p className="text-[10px] font-bold tracking-[0.2em] uppercase mb-2" style={{ color: '#3B82F6' }}>Equimon · Student Portal</p>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-full lg:w-[720px] z-50 shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
      ...
      >
        <div className="relative w-full h-full overflow-hidden">
          ...
          <div
            ...
            className={`absolute inset-0 flex flex-col justify-center px-10 pb-10 pt-16 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
              signupStep === 0 ? 'opacity-100 translate-x-0 pointer-events-auto' : 'opacity-0 -translate-x-full pointer-events-none'
            }`}
          >
            <div className="w-full flex flex-col items-center text-center" style={{ animation: 'su-fadeInUp 0.5s ease 0.05s both' }}>
              ...
              {/* Hero title */}
              <p className="text-xs font-bold tracking-[0.22em] uppercase mb-3" style={{ color: '#3B82F6' }}>Equimon</p>
              <h1 className="text-4xl font-bold text-white leading-tight mb-4">Create Your Account</h1>
              <p className="text-sm leading-relaxed max-w-md mb-10" style={{ color: '#64748B' }}>
                {t('equimonHelps')}
              </p>

              {/* Feature cards */}
              <div className="w-full grid grid-cols-3 gap-4 mb-10">
                {featureItems.map(({ icon: Icon, label, desc }, i) => (
                  <div
                    key={i}
                    className="flex flex-col items-center text-center p-5 rounded-2xl"
                    style={{ background: 'rgba(17,17,24,0.7)', border: '1px solid rgba(30,41,59,0.6)' }}
                  >
                    <div
              ...
              {/* Divider */}
              <div className="w-full h-px mb-7" style={{ background: 'linear-gradient(to right, transparent, rgba(59,130,246,0.2), transparent)' }} />
              ...
              <div className="w-full flex flex-col gap-3">
                ...
                <button
                  type="button"
                  onClick={closeSignupModal}
                  className="lp-btn-ghost w-full h-12 rounded-xl font-semibold text-sm"
                >
                  {t('forgotPasswordBack')}
                </button>
              </div>
          ...
          {/* ── STEP 1: Sign-Up Form ── */}
          <div
            className={`absolute inset-0 overflow-y-auto flex flex-col items-center justify-start px-10 pt-12 pb-6 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          ...
          >
            <div className="w-full">
              ...
              <form onSubmit={handleSignupSubmit} className="space-y-6" autoComplete="off">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          autoComplete="off"
                          className={`su-input w-full pl-10 pr-3 py-2.5 rounded-xl text-sm ${signupErrors.lastName ? 'su-input-err' : ''}`}
                        />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          autoComplete="off"
                          className={`su-input w-full pl-10 pr-3 py-2.5 rounded-xl text-sm ${signupErrors.firstName ? 'su-input-err' : ''}`}
                        />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        autoComplete="off"
                        className={`su-input w-full pl-10 pr-4 py-2.5 rounded-xl text-sm ${signupErrors.middleName ? 'su-input-err' : ''}`}
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        pattern="[0-9]*"
                        autoComplete="off"
                        className={`su-input w-full pl-10 pr-4 py-2.5 rounded-xl text-sm ${signupErrors.studentId ? 'su-input-err' : ''}`}
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        autoComplete="off"
                        className={`su-input w-full pl-10 pr-4 py-2.5 rounded-xl text-sm ${signupErrors.department ? 'su-input-err' : ''}`}
                      />
                    </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t('placeholderEmail')}
                      autoComplete="off"
                      className={`su-input w-full pl-10 pr-4 py-2.5 rounded-xl text-sm ${signupErrors.email ? 'su-input-err' : ''}`}
                    />
                  </div>
                  {signupErrors.email
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={t('placeholderPassword')}
                        autoComplete="new-password"
                        className={`su-input w-full pl-10 pr-11 py-2.5 rounded-xl text-sm ${signupErrors.password ? 'su-input-err' : ''}`}
                      />
                      <button
                        type="button"
                    ...
                    {!signupErrors.password && signupForm.password && (
                      <div className="mt-2 space-y-1">
                        <div className="flex items-center justify-between">
                          <p className="text-xs" style={{ color: '#475569' }}>Strength</p>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        autoComplete="new-password"
                        className={`su-input w-full pl-10 pr-4 py-2.5 rounded-xl text-sm ${signupErrors.confirmPassword ? 'su-input-err' : ''}`}
                      />
                    </div>
                ...
                <div className="flex flex-col gap-2 mb-12">
                  <button
                    ...
                    className="lp-btn-primary w-full py-2.5 h-11 rounded-xl font-semibold text-white text-sm flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    type="button"
                    onClick={closeSignupModal}
                    className="lp-btn-ghost w-full py-2.5 h-11 rounded-xl font-semibold text-sm"
                  >
                    {t('forgotPasswordBack')}
                  </button>
                </div>
