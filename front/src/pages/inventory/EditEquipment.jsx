  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (imageFiles.length > 0) {
        ...
        // Proxy any external URLs (http/https) through backend GridFS
        for (let i = 0; i < uploadedUrls.length; i++) {
          if (/^https?:\/\//i.test(uploadedUrls[i])) {
            try {
              uploadedUrls[i] = await api.entities.Equipment.uploadImageFromUrl(uploadedUrls[i]);
            } catch {
              // Keep original URL on failure (best-effort)
            }
          }
        }
      }
