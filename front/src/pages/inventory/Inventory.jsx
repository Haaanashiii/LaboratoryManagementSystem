...
const EquipmentThumb = ({ url, name }) => {
  const [failed, setFailed] = useState(false);
  if (!url || failed) return <Package style={{ width: 16, height: 16, color: 'var(--a-mute-2)' }} />;
  return (
    <img
      src={url}
      alt={name}
...
const categoryConfig = [
  { value: 'Electronics',  labelKey: 'catElectronics',  icon: Cpu       },
  { value: 'Computing',    labelKey: 'catComputing',    icon: Monitor   },
  { value: 'Networking',   labelKey: 'catNetworking',   icon: Network   },
  { value: 'Peripherals',  labelKey: 'catPeripherals',  icon: Mouse     },
  { value: 'Storage',      labelKey: 'catStorage',      icon: HardDrive },
  { value: 'Cables',       labelKey: 'catCables',       icon: Cable     },
  { value: 'Tools',        labelKey: 'catTools',        icon: Wrench    },
  { value: 'Other',        labelKey: 'catOther',        icon: Boxes     },
];
...
export default function Inventory() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDeleteItem(null);
      toast.success('Equipment and all associated records deleted.');
    },
    onError: (err) => toast.error(err?.message || 'Failed to delete equipment.'),
  });
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['equipment'] });
      toast.success(data?.isPublished === false ? t('hiddenFromStudents') : t('visibleToStudents'));
    },
    onError: (err) => toast.error(err?.message || t('failedToConnect')),
  });
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px', minHeight: '100%' }}>
      ...
      <div className="a-titlestrip">
        <div>
          <div className="a-eyebrow">{t('platformInventoryEyebrow')}</div>
          <h1>{t('inventory')}</h1>
          <div className="a-deck">{t('inventoryDeck')}</div>
        </div>
        <div className="a-right">
          <span style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--a-mute)' }}>
            {equipment.length} {t('itemsLabel')} · {equipment.filter(e => (e.available_quantity ?? 0) > 0).length} {t('availableLabel').toLowerCase()}
          </span>
          {canAdd && (
      ...
      <div className="a-panel">
        <div className="p-head">
          <h2>{activeCatConfig ? t(activeCatConfig.labelKey) : t('allEquipment')}</h2>
        ...
        {isError ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '64px 24px', gap: 8 }}>
            ...
            <p style={{ fontFamily: 'var(--serif)', color: 'var(--a-ink)', margin: 0 }}>{t('unableLoadEquipment')}</p>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="a-table">
              <thead>
                <tr>
                  ...
                  <th className="num">{t('availableLabel')}</th>
                  <th>{t('condition')}</th>
                  <th>{t('visibility')}</th>
                  {canEdit && <th>{t('tableColActions')}</th>}
                </tr>
              </thead>
              <tbody>
                      ...
                      <tr key={item.id}>
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ width: 36, height: 36, border: '1px solid var(--a-rule)', background: 'var(--a-surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                              <EquipmentThumb url={item.image_url} name={item.name} />
                            </div>
                            <div>
                        ...
                        <td>
                          <span className={`a-pill ${condPill}`}>
                            ...
                            {item.condition || t('good')}
                          </span>
                        </td>
                        <td>
                          {item.isPublished === false ? (
                            <span className="a-pill p-mute">{t('hidden')}</span>
                          ) : (
                            <span className="a-pill p-ok">
                              <span className="dot" style={{ background: 'currentColor' }} />
                              {t('inventoryActionPublished')}
                            </span>
                          )}
                        </td>
                        {canEdit && (
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                              {['admin', 'head_of_lab', 'lab_assistant'].includes(user?.role) && (
                                <button
                                  type="button"
                                  title={item.isPublished === false ? t('inventoryActionPublished') : t('inventoryActionUnpublished')}
                              ...
                              <button
                                className="a-btn"
                                style={{ padding: '5px 8px', color: 'var(--a-navy)' }}
                              ...
                              <button
                                className="a-btn"
                                style={{ padding: '5px 8px', color: 'var(--a-bad)' }}
            ...
            {totalPages > 1 && (
              <div className="a-pagination">
                <span className="info">
                  {t('showing')} {(currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, displayedEquipment.length)} {t('ofLabel')} {displayedEquipment.length}
                </span>
                <div className="pages">
        ...
        <DialogContent className="sm:max-w-md rounded-2xl bg-white text-slate-900">
          <DialogHeader className="border-b border-slate-100 pb-4">
            <DialogTitle className="flex items-center gap-2.5 text-base font-semibold">
              ...
              {t('deleteEquipmentTitle')}
            </DialogTitle>
          </DialogHeader>
          <div className="py-4 flex flex-col gap-3">
            <p className="text-sm text-slate-500 leading-relaxed">
              {t('aboutToPermanentlyDelete')}{' '}
              <span className="font-medium text-slate-700">"{deleteItem?.name}"</span>.{' '}
              {t('thisActionCannotBeUndone')}
            </p>
            <div style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '10px 12px' }}>
              <p style={{ fontSize: 12, color: '#b91c1c', fontWeight: 600, margin: '0 0 4px' }}>Also deletes permanently:</p>
              <ul style={{ fontSize: 12, color: '#dc2626', margin: 0, paddingLeft: 16, lineHeight: 1.7 }}>
                <li>All borrow request history for this equipment</li>
                <li>Uploaded images stored in the system</li>
              </ul>
              <p style={{ fontSize: 11, color: '#ef4444', margin: '6px 0 0', fontStyle: 'italic' }}>
                Deletion is blocked if any borrow is still active.
              </p>
            </div>
          </div>
          <DialogFooter className="gap-2 border-t border-slate-100 pt-4">
            <Button variant="outline" size="sm" onClick={() => setDeleteItem(null)} className="text-xs">{t('cancel')}</Button>
            <Button
              size="sm"
            ...
            >
              {deleteMutation.isPending
                ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />{t('deleting')}</>
                : t('deleteEquipmentTitle')}
            </Button>
          </DialogFooter>
        </DialogContent>
