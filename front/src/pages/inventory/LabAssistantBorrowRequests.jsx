...
const STATUS_CFG = {
  pending:          { labelKey: 'pending',          stripe: '#f59e0b', bg: '#fffbeb', text: '#b45309', border: '#fde68a', dot: '#f59e0b' },
  pending_lecturer: { labelKey: 'pendingLecturer',  stripe: '#f59e0b', bg: '#fffbeb', text: '#b45309', border: '#fde68a', dot: '#f59e0b' },
  pending_head:     { labelKey: 'pendingHead',      stripe: '#f59e0b', bg: '#fffbeb', text: '#b45309', border: '#fde68a', dot: '#f59e0b' },
  approved:         { labelKey: 'approved',         stripe: '#16a34a', bg: '#f0fdf4', text: '#15803d', border: '#bbf7d0', dot: '#22c55e' },
  rejected:         { labelKey: 'rejected',         stripe: '#ef4444', bg: '#fef2f2', text: '#b91c1c', border: '#fecaca', dot: '#ef4444' },
  head_approved:    { labelKey: 'headapproved',     stripe: '#3b82f6', bg: '#eff6ff', text: '#1d4ed8', border: '#bfdbfe', dot: '#3b82f6' },
  ready_pickup:     { labelKey: 'readyPickup',      stripe: '#8b5cf6', bg: '#f5f3ff', text: '#6d28d9', border: '#ddd6fe', dot: '#8b5cf6' },
  borrowed:         { labelKey: 'borrowed',         stripe: '#06b6d4', bg: '#ecfeff', text: '#0e7490', border: '#a5f3fc', dot: '#06b6d4' },
  returned:         { labelKey: 'returned',         stripe: '#64748b', bg: '#f8fafc', text: '#475569', border: '#e2e8f0', dot: '#64748b' },
};

const getStatusCfg = (s) => STATUS_CFG[s] || { labelKey: null, stripe: '#64748b', bg: '#f8fafc', text: '#475569', border: '#e2e8f0', dot: '#64748b' };

const TAB_KEYS = [
  { key: 'pending', labelKey: 'pending', icon: Clock },
  { key: 'all',     labelKey: 'all',     icon: LayoutList },
];
  ...
  return (
    <div className="w-full space-y-4 px-2 py-2">
      ...
      <EquimonPageHeader
        accent={ACCENT}
        icon={FileText}
        title={t('borrowRequests')}
        badgeCount={pendingCount}
        badge={t('pending')}
        sub={t('reviewBorrowRequestsSub')}
      />
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder={t('searchByNameEquipment')}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-20">
          ...
          <p className="text-sm text-slate-500">{t('failedToLoadRequests')}</p>
          <button onClick={() => refetch()} className="text-xs text-teal-600 underline">{t('retry')}</button>
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-20">
          <Package className="w-10 h-10 text-slate-300" />
          <p className="text-sm font-medium text-slate-500">
            {tab === 'pending' ? t('noPendingRequestsFound') : t('noRequestsFound')}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {paginated.map((r) => {
            ...
            return (
              <div
                ...
                onClick={() => { const _eqLab = r?.equipment?._id || r?.equipment?.id || r?.equipment; const _eqDataLab = _eqLab ? equipmentMap[_eqLab] : null; setSelected({ ...r, _resolvedImageUrl: resolveImageUrl(r), assistant_name: r.released_by?.name || r.prepared_by?.name || r.assistant_name || user?.name, category: r.category || _eqDataLab?.category }); }}
              >
                {/* Left stripe */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mb-1">
                      ...
                      >
                        ...
                        {sc.labelKey ? t(sc.labelKey) : r.status}
                      </span>
                      {category && (
