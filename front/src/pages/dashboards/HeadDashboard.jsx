...
export default function HeadDashboard() {
  ...
  const kpiCards = [
    { label: t('pendingFinalApprovalKpiLabel'), value: pendingApprovals.length, sub: `${recentRequests.filter(r => r.status === 'pending_head').length} ${t('newThisWeek')}`, icon: Clock, color: '#f59e0b', onClick: () => navigate('/head-approvals') },
    { label: t('approvedRequestsKpiLabel'), value: approvedRequests.length, sub: `${approvalRate}${t('approvalRatePct')}`, icon: CheckCircle, color: '#22c55e', onClick: () => navigate('/head-approval-history') },
    { label: t('totalEquipmentKpiLabel'), value: totalEquipment, sub: `${availableEquipment} ${t('currentlyAvailableSuffix')}`, icon: Package, color: '#2563eb', onClick: () => navigate('/inventory') },
    { label: t('availableNowKpi'), value: availableEquipment, sub: `${totalEquipment - availableEquipment} ${t('inUseSuffix')}`, icon: BarChart3, color: '#7c3aed', onClick: () => navigate('/inventory') },
  ];
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px', minHeight: '100%' }}>
      ...
      <div className="a-titlestrip">
        <div>
          <div className="a-eyebrow">{t('headOfLabDashEyebrow')}</div>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          ...
          {pendingApprovals.slice(0, 3).map((r, i) => (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: '1px solid var(--a-rule-2)' }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(37,99,235,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Clock style={{ width: 16, height: 16, color: '#2563eb' }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontFamily: 'var(--sans)', fontWeight: 600, fontSize: 13, color: 'var(--a-ink)' }}>{t('awaitingFinalApprovalAction')}</span>
              ...
              <button className="a-btn primary" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4 }} onClick={() => navigate('/head-approvals')}>
      ...
      <div className="a-panel">
        <div className="p-head">
          <h2>{t('requestsAwaitingFinalApproval')}</h2>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          ...
          <div style={{ padding: '16px 20px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 15, color: 'var(--a-mute)' }}>{t('approvedStatLabel')}</span>
            </div>
            ...
            {activityChartData.every(d => d.pending === 0 && d.approved === 0 && d.borrowed === 0 && d.returned === 0) ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
                  {[
                    { labelKey: 'pending',          color: 'var(--a-gold)' },
                    { labelKey: 'approvedStatLabel', color: '#2563eb' },
                    { labelKey: 'borrowed',         color: 'var(--a-ok)' },
                    { labelKey: 'returned',         color: '#94a3b8' },
                  ].map(({ labelKey, color }) => (
