...
const STATUS_LABELS = {
  pending_lecturer: 'Pending Lecturer', pending_head: 'Pending Head',
  head_approved: 'Approved', ready_pickup: 'Ready Pickup',
  borrowed: 'Borrowed', returned: 'Returned', rejected: 'Rejected',
};
...
const ROLE_LABELS = {
  student: 'Student', lecturer: 'Lecturer', lab_assistant: 'Lab Assistant',
  head_of_lab: 'Head of Lab', admin: 'Admin',
};
  ...
  return (
    <div style={{ minWidth: 220, borderRadius: 8, border: '1px solid var(--a-rule)', background: '#fff', padding: '12px 14px', boxShadow: '0 8px 24px rgba(0,0,0,0.10)', fontSize: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--a-rule-2)', paddingBottom: 8, marginBottom: 10 }}>
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--a-mute)', background: 'var(--a-rule-2)', padding: '2px 6px' }}>{total} total</span>
      </div>
          ...
          <div key={entry.name} style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <strong style={{ color: 'var(--a-ink)' }}>{entry.name}</strong>
              </div>
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px', minHeight: '100%' }}>
      ...
      <div className="a-titlestrip">
        <div>
          <div className="a-eyebrow">Admin · Overview</div>
          <h1>
            {gc.greeting},{' '}
            <span style={{ color: 'var(--a-gold)' }}>{currentUser?.name?.split(' ')[0] || 'Admin'}</span>
          </h1>
        ...
        <div className="a-right">
          {pendingDamageReports.length > 0 && (
            ...
            >
              ...
              {pendingDamageReports.length} damage {pendingDamageReports.length !== 1 ? 'reports' : 'report'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="a-cards">
        {[
          { title: 'Total Users',      value: totalUsers,      icon: Users,     sub: 'Registered accounts',          onClick: () => navigate('/users') },
          { title: 'Total Equipment',  value: totalEquipment,  icon: Package,   sub: `${availableEquipment} available`, onClick: () => navigate('/inventory') },
          { title: 'Active Requests',  value: activeRequests,  icon: FileText,  sub: `${totalRequests} total requests`, onClick: () => navigate('/all-requests') },
          { title: 'Utilization',      value: `${utilizationRate}%`, icon: Activity, sub: 'Equipment in use',        onClick: () => navigate(CATALOG_ROUTES_BY_ROLE.admin) },
      ...
      <div className="a-panel">
        <div className="p-head">
          <div style={{ width: 36, height: 36, background: 'var(--a-navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <PackageCheck style={{ width: 16, height: 16, color: 'var(--a-gold)' }} />
          </div>
          <h2>Equipment Needs to Be Marked Ready</h2>
          <span className="count">{needsReadyItems.length} pending</span>
          <div className="spacer" />
          <span style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--a-mute)' }}>
            Head-approved · awaiting lab assistant
          </span>
        </div>
        {needsReadyItems.length === 0 ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '28px 24px' }}>
            ...
            <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', color: 'var(--a-mute)', fontSize: 13 }}>All equipment has been prepared</span>
          </div>
        ) : (
              ...
              <tbody>
                {needsReadyItems.map((item, i) => (
                  <tr key={item.id || item._id}>
                    ...
                    <td><span className="a-pill p-info"><span className="dot" style={{ background: 'currentColor' }} />Needs Prep</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="a-panel" style={{ marginBottom: 20 }}>
        <div style={{ padding: '20px 24px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 24 }}>
          <div style={{ flexShrink: 0 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--a-mute)' }}>Equipment Utilization</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexShrink: 0 }}>
            {[
              { label: 'Borrowed', value: totalEquipment - availableEquipment, color: 'var(--a-navy)' },
              { label: 'Available', value: availableEquipment, color: 'var(--a-ok)' },
              { label: 'Total', value: totalEquipment, color: 'var(--a-ink)' },
      ...
      {overdueItems.length > 0 ? (
        <div className="a-panel" id="overdue-section" style={{ borderColor: 'var(--a-bad)', marginBottom: 20 }}>
          <div className="p-head" style={{ background: 'var(--a-bad-bg)' }}>
            ...
            {(() => {
              ...
              return (
                <>
                  {critical > 0 && <span className="a-pill p-bad"><span className="dot" style={{ background: 'currentColor' }} />{critical} critical</span>}
                  {warning  > 0 && <span className="a-pill p-warn"><span className="dot" style={{ background: 'currentColor' }} />{warning} warning</span>}
                </>
              );
            })()}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, border: '1px solid var(--a-ok)', background: 'var(--a-ok-bg)', padding: '16px 20px', marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontFamily: 'var(--serif)', fontWeight: 600, color: 'var(--a-ok)', fontSize: 14 }}>No overdue returns</div>
            <div style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', color: 'var(--a-ok)', fontSize: 12, marginTop: 2 }}>All borrowed items are within their return window</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: '2fr 1fr', marginBottom: 16 }}>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          <div className="p-head">
            ...
            <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 11, color: 'var(--a-mute)' }}>Last {activityDays} days</span>
            <div className="spacer" />
            ...
            {activityChartData.every(d => d.new === 0 && d.active === 0 && d.done === 0) ? (
              ...
              <>
                ...
                <div style={{ marginTop: 12, display: 'flex', gap: 20, flexWrap: 'wrap' }}>
                  {Object.values(ACTIVITY_CHART_CONFIG).map(({ label, color }) => (
                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--a-mute)' }}>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          <div className="p-head"><h2>Request Status</h2></div>
          <div style={{ padding: '16px 20px' }}>
            {requestStatusData.length === 0 ? <EmptyState message="No requests yet" /> : (
              <>
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: '2fr 1fr', marginBottom: 16 }}>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          ...
          <div style={{ padding: '16px 20px' }}>
            {mostBorrowedChartData.length === 0 ? <EmptyState message="No borrow data yet" /> : (
                ...
                {mostBorrowedChartData.map((item, i) => {
                  ...
                  return (
                    <div key={item.name} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <span style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isTop ? 'var(--a-navy)' : 'var(--a-surface-2)', color: isTop ? 'var(--a-gold)' : 'var(--a-mute)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{i + 1}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* Users by Role */}
        <div className="a-panel" style={{ marginBottom: 0 }}>
          <div className="p-head"><h2>Users by Role</h2></div>
          <div style={{ padding: '16px 20px' }}>
            {userRoleData.length === 0 ? <EmptyState message="No user data" /> : (
              <>
      ...
      {pendingDamageReports.length > 0 && (
        <div className="a-panel" id="damage-section" style={{ borderColor: 'var(--a-gold)', marginBottom: 0 }}>
          <div className="p-head" style={{ background: 'var(--a-warn-bg)' }}>
            ...
            <h2>Pending Damage Verification</h2>
            <span className="count">{pendingDamageReports.length} pending</span>
          </div>
          <div>
            {pendingDamageReports.slice(0, 5).map(item => (
              <div key={item.id} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '16px 20px', borderBottom: '1px solid var(--a-warn-bg)' }}>
                <div style={{ minWidth: 0 }}>
                  <div className="primary">{item.equipment_name}</div>
                  <div className="muted" style={{ fontSize: 11 }}>Borrower: {item.borrower_name}</div>
