...
export default function LecturerDashboard() {
  ...
  const kpiCards = [
    { label: 'PENDING APPROVALS', value: pendingApprovals.length, sub: `${recentRequests.filter(r => r.status === 'pending_lecturer').length} new this week`, icon: Clock, color: '#f59e0b', onClick: () => navigate('/lecturer-approvals') },
    { label: 'APPROVED BY ME', value: approvedByMe.length, sub: `${approvalRate}% approval rate`, icon: CheckCircle, color: '#2563eb', onClick: () => navigate('/lecturer-approval-history') },
    { label: 'TOTAL REQUESTS', value: allRequests.length, sub: `${recentRequests.length} in last 7 days`, icon: Package, color: '#7c3aed', onClick: () => navigate('/lecturer-approval-history') },
    { label: 'REJECTED', value: rejectedByMe.length, sub: `${rejectionRate}% rejection rate`, icon: AlertCircle, color: '#ef4444', onClick: () => navigate('/lecturer-approval-history') },
  ];
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px', minHeight: '100%' }}>
      ...
      <div className="a-titlestrip">
        <div>
          <div className="a-eyebrow">Lecturer · Dashboard</div>
            ...
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: '1px solid var(--a-rule-2)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontFamily: 'var(--sans)', fontWeight: 600, fontSize: 13, color: 'var(--a-ink)' }}>Pending approval</span>
              ...
              <button className="a-btn gold" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 4 }} onClick={() => navigate('/lecturer-approvals')}>
        ...
        <div className="a-panel" style={{ marginBottom: 0 }}>
          ...
          <div style={{ padding: '16px 20px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 15, color: 'var(--a-mute)' }}>Forwarded</span>
            </div>
              ...
              <>
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={activityChartData} margin={{ top: 4, right: 4, bottom: 0, left: -28 }}>
                    ...
                    <Area type="monotone" dataKey="new" name="Pending" stackId="a" stroke="var(--a-gold)" strokeWidth={1.5} fill="var(--a-gold)" fillOpacity={0.2} dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
                <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
                  {[{ label: 'Pending', color: 'var(--a-gold)' }, { label: 'Approved', color: 'var(--a-ok)' }, { label: 'Rejected', color: '#ef4444' }].map(({ label, color }) => (
