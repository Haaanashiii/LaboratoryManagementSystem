      ...
      <div className={`fade-up fade-up-5 rounded-2xl border shadow-sm overflow-hidden transition-colors ${isDark ? 'border-white/[0.08] hover:border-white/[0.14] bg-[#0d0d14]' : 'border-stone-300 hover:border-stone-400 bg-stone-100'}`}>
        <div className={`flex items-center justify-between px-6 pt-5 pb-4 border-b ${isDark ? 'border-white/10' : 'border-stone-200'}`}>
          <div className="flex items-center gap-2">
            ...
            <h2 className={`text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{t('myRecentRequests')}</h2>
          </div>
          {myRequests.length > 0 && (
            <button
        ...
        {myRequests.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center px-6">
            ...
            <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>No requests yet. Start borrowing!</p>
            ...
            {myRequests.slice(0, 6).map((request, idx) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 border ${isDark ? 'bg-white/5 border-white/10' : 'bg-gradient-to-br from-blue-50 to-indigo-50 border-slate-100'}`}>
                    <Box className="w-4 h-4 text-blue-500" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={`font-semibold text-sm truncate ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{request.equipment_name}</p>
                    <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                      Qty <span className={`font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{request.quantity}</span>
