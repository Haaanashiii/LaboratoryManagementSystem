...
export default function BorrowReports() {
  ...
  const filtered = requests.filter(r => {
    if (r.equipment === null || r.equipment === undefined) return false;
                          ...
                          <tr key={request.id}>
                            ...
                            <td>
                              <span className={`a-pill ${pillClass}`} style={{ whiteSpace: 'nowrap' }}>
                                {STATUS_LABEL_KEYS[request.status] ? t(STATUS_LABEL_KEYS[request.status]) : request.status}
                              </span>
                            </td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              <button
                                className="a-btn"
                                onClick={() => { const _eqRpt = request?.equipment?._id || request?.equipment?.id || request?.equipment; const _eqDataRpt = _eqRpt ? equipmentMap[_eqRpt] : null; setReportRequest({ ...request, category: request.category || _eqDataRpt?.category }); }}
                                style={{ gap: 5 }}
                              >
