...
function inventoryActionLabel(action, t) {
  switch (action) {
    case 'added':            return t('inventoryActionAdded');
    case 'deleted':          return t('inventoryActionDeleted');
    case 'quantity_changed': return t('inventoryActionQtyChanged');
    case 'published':        return t('inventoryActionPublished');
    case 'unpublished':      return t('inventoryActionUnpublished');
    default:                 return action || '—';
  }
}

function inventoryDetails(record, t) {
  if (record.action === 'added')            return `${t('initialQtyLabel')}: ${record.newQuantity ?? '—'}`;
  if (record.action === 'deleted')          return `${t('hadQtyLabel')}: ${record.previousQuantity ?? '—'}`;
  if (record.action === 'quantity_changed') return `${record.previousQuantity ?? '—'} → ${record.newQuantity ?? '—'}`;
  if (record.action === 'published')        return t('visibleToStudents');
  if (record.action === 'unpublished')      return t('hiddenFromStudents');
  return '—';
}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${active ? 'var(--a-navy)' : 'var(--a-rule)'}`,
        background: active ? 'rgba(30,58,138,0.08)' : 'transparent',
        color: active ? 'var(--a-navy)' : 'var(--a-mute)',
      }}
    >
      {children}
    </button>
...
export default function Reports() {
  ...
  const TABS = [
    ...
    { key: 'inventory', label: t('inventoryChangesTab'), icon: <PackagePlus style={{ width: 14, height: 14 }} /> },
  ];

  const statCards = activeTab === 'inventory'
    ? [
        { label: t('itemsAddedLabel'),            value: inventorySummary.totalAdded },
        { label: t('itemsDeletedLabel'),          value: inventorySummary.totalDeleted },
        { label: t('qtyChangesLabel'),            value: inventorySummary.totalQuantityChanges },
        { label: t('inventoryActionPublished'),   value: inventorySummary.totalPublished },
        { label: t('inventoryActionUnpublished'), value: inventorySummary.totalUnpublished },
      ]
    : [
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px 40px' }}>
      ...
      <div className="a-titlestrip">
        <div style={{ flex: 1 }}>
          <div className="a-eyebrow">{t('inventoryLendingReportsEyebrow')}</div>
      ...
      <div style={{ padding: '0 24px' }}>
        ...
        <div className="a-panel" style={{ marginTop: 16 }}>
          <div className="a-filters">
            ...
            <div className="a-field">
              ...
              <input type="date" value={startDate} max={new Date().toISOString().split('T')[0]} onChange={(e) => { setStartDate(e.target.value); setCurrentPage(1); }} />
            </div>

            <div className="a-field">
              <label>{t('endDate')}</label>
              <input type="date" value={endDate} max={new Date().toISOString().split('T')[0]} onChange={(e) => { setEndDate(e.target.value); setCurrentPage(1); }} />
            </div>
          ...
          <div className="a-export">
            <div className="e focus">
              ...
              <div className="e-actions">
                ...
                <button className="a-btn" onClick={() => exportXlsAll(allRecords)} disabled={isFetching || allRecords.length === 0}>
            ...
            <div className="e">
              ...
              <div className="e-actions">
                ...
                <button className="a-btn" onClick={() => exportXlsWeekly(weeklySummary)} disabled={isFetching || weeklySummary.length === 0}>
                  <FileSpreadsheet style={{ width: 13, height: 13 }} /> Excel
                </button>
              </div>
            </div>
            ...
            <div className="e">
              <div className="e-title">{t('inventoryChangesReport')}</div>
              <div className="e-desc">{t('inventoryChangesReportDesc')}</div>
              <div className="e-actions">
                ...
                <button className="a-btn" onClick={() => exportXlsInventory(inventoryRecords)} disabled={inventoryLoading || inventoryRecords.length === 0}>
                  <FileSpreadsheet style={{ width: 13, height: 13 }} /> Excel
                </button>
              </div>
            </div>
        ...
        {!error && activeTab === 'weekly' && (
          <div className="a-panel" style={{ marginTop: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: '1px solid var(--a-rule)' }}>
              ...
              <span className="a-pill p-mute">{weeklySummary.length} {weeklySummary.length !== 1 ? t('weeksLabel') : t('weekLabel')}</span>
            </div>
            ...
            {weeklySummary.length === 0 ? (
              ...
              <>
                ...
                {totalWeekPages > 1 && (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--a-rule)' }}>
                    <span style={{ fontSize: 12, color: 'var(--a-mute)' }}>
                      {t('showing')} {(weekPage - 1) * PAGE_SIZE + 1}–{Math.min(weekPage * PAGE_SIZE, weeklySummary.length)} {t('ofLabel')} {weeklySummary.length} {weeklySummary.length !== 1 ? t('weeksLabel') : t('weekLabel')}
                    </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                {['', 'pending', 'approved', 'rejected', 'borrowed', 'returned'].map((s) => (
                  <Chip key={s} active={borrowStatusFilter === s} onClick={() => { setBorrowStatusFilter(s); setBorrowPage(1); }}>
                    {s === '' ? t('all') : t(STATUS_COLORS[s]?.labelKey) || s}
                  </Chip>
                ))}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button
                  className="a-btn"
                  onClick={() => exportXlsBorrow(filteredBorrowRequests)}
        ...
        {activeTab === 'inventory' && (
          <div className="a-panel" style={{ marginTop: 12 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '12px 16px', borderBottom: '1px solid var(--a-rule)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <PackagePlus style={{ width: 15, height: 15, color: '#3b82f6' }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--a-ink)' }}>{t('inventoryChangesTab')}</span>
                <span className="a-pill p-mute">{inventoryRecords.length}</span>
                {[
                  { value: '', labelKey: 'all' },
                  { value: 'added', labelKey: 'inventoryActionAdded' },
                  { value: 'deleted', labelKey: 'inventoryActionDeleted' },
                  { value: 'quantity_changed', labelKey: 'inventoryActionQtyChanged' },
                  { value: 'published', labelKey: 'inventoryActionPublished' },
                  { value: 'unpublished', labelKey: 'inventoryActionUnpublished' },
                ].map((opt) => (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="a-btn" onClick={() => exportXlsInventory(inventoryRecords)} disabled={inventoryRecords.length === 0}>
                  <FileSpreadsheet style={{ width: 13, height: 13 }} /> Excel
                </button>
            ...
            {inventoryLoading ? (
              <div style={{ textAlign: 'center', padding: 64, fontSize: 13, color: 'var(--a-mute)' }}>{t('loadingLabel')}</div>
            ) : paginatedInventoryRecords.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 64, fontSize: 13, color: 'var(--a-mute)' }}>{t('noInventoryChangesFound')}</div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table className="a-table">
                  <thead>
                  ...
                  <tbody>
                    {paginatedInventoryRecords.map((r, idx) => (
                      <tr key={r._id || idx}>
                        ...
                        <td><span className={`a-pill ${inventoryActionPill(r.action)}`}>{inventoryActionLabel(r.action, t)}</span></td>
                        <td style={{ fontSize: 12, color: 'var(--a-ink-2)' }}>{inventoryDetails(r, t)}</td>
