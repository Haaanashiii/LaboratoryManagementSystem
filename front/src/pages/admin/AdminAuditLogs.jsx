...
import { printAuditTrailReport } from '@/utils/printEquimonReport';
...
export default function AdminAuditLogs() {
  ...
  const KPI_TABS = [
    { key: 'all',           label: t('allLogs'),              count: counts.all,           Icon: LayoutList   },
    { key: 'login_success', label: t('loginSuccess'),          count: counts.login_success, Icon: CheckCircle2 },
    { key: 'login_failed',  label: t('loginFailed'),           count: counts.login_failed,  Icon: AlertTriangle },
    { key: 'sensitive',     label: t('sensitiveActionsLabel'), count: counts.sensitive,     Icon: Shield       },
  ];
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px 40px' }}>
      ...
      <div className="a-titlestrip">
        <div style={{ flex: 1 }}>
          <div className="a-eyebrow">{t('securityComplianceEyebrow')}</div>
          <h1>{t('systemAuditTrailTitle')}</h1>
          <p className="a-deck">{t('reviewExportLogsDesc')}</p>
        </div>
        <div className="a-right">
          <button className="a-btn gold" onClick={handleExportPdf}>
        ...
        <div className="a-panel" style={{ marginTop: 16 }}>
          <div className="a-filters" style={{ padding: '14px 18px' }}>
            ...
            <div className="a-field">
              ...
              <input type="date" value={startDateInput} max={new Date().toISOString().split('T')[0]} onChange={(e) => setStartDateInput(e.target.value)} />
            </div>
            <div className="a-field">
              <label>{t('endDate')}</label>
              <input type="date" value={endDateInput} max={new Date().toISOString().split('T')[0]} onChange={(e) => setEndDateInput(e.target.value)} />
            </div>
        ...
        <div className="a-panel" style={{ marginTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid var(--a-rule)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--a-ink)' }}>{t('logEntriesLabel')}</span>
            ...
            <span style={{ fontSize: 11, color: 'var(--a-mute)', fontFamily: 'var(--mono)' }}>
              {t('showing')} {PAGE_SIZE} {t('perPageLabel')}
            </span>
          </div>
            ...
            <table className="a-table">
              ...
              <tbody>
                {displayedLogs.length === 0 ? (
                  ...
                  const email      = log.user?.email || log.user_email || log.details?.email || '';
                  const isDeleted  = !log.user && !name && log.user_email;
                    ...
                    <tr key={log._id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: name ? 'var(--a-ink)' : 'var(--a-mute)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>
                              {name || (isDeleted ? t('deletedAccount') : t('unknownUser'))}
                            </div>
                            {email ? (
                              <div style={{ fontSize: 11, color: 'var(--a-mute)', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>{email}</div>
                            ) : !name && log.action_type === 'login_failed' ? (
                              <div style={{ fontSize: 10, color: 'var(--a-mute)', fontStyle: 'italic' }}>{t('attemptedLogin')}</div>
                            ) : null}
                          </div>
                        </div>
                      </td>
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 500, color: isSuccess ? 'var(--a-ok)' : 'var(--a-warn)' }}>
                          ...
                          {isSuccess ? t('successLabel') : (log.status || t('failedLabel'))}
                        </div>
                      </td>
          ...
          {displayedLogs.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderTop: '1px solid var(--a-rule)' }}>
              <span style={{ fontSize: 12, color: 'var(--a-mute)' }}>
                {t('showing')} {Math.min((currentPage - 1) * PAGE_SIZE + 1, displayedLogs.length)}–{Math.min(currentPage * PAGE_SIZE, displayedLogs.length)} {t('ofLabel')} {displayedLogs.length.toLocaleString()} {t('entriesLabel')}
              </span>
