...
const STATUS_FILTERS = [
  { key: 'all',      label: 'All',      statuses: null,             color: 'bg-slate-50 text-slate-700 border-slate-200',    dot: '#64748b', icon: LayoutList  },
  { key: 'approved', label: 'Approved', statuses: APPROVED_STATUSES, color: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: '#22c55e', icon: CheckCircle },
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold tracking-tight text-slate-900">{t('approvalHistory')}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2.5 rounded-xl border border-violet-100 bg-violet-50 px-4 py-2.5">
          ...
          <div className="text-left">
            <p className="text-base font-bold leading-none tabular-nums text-violet-700">{myActedRequests.length}</p>
            <p className="mt-0.5 text-[10px] font-medium text-violet-500">
              total record{myActedRequests.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="overflow-hidden border-slate-200 shadow-none">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-slate-800">{activeFilter.label}</p>
        ...
        <CardContent className="p-0">
          {isError ? (
            <div className="flex flex-col items-center justify-center gap-2 py-16">
              ...
              <p className="text-sm font-medium text-slate-800">{t('unableLoadHistory')}</p>
