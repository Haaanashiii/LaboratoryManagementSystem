...
const STATUS_FILTERS = [
  { key: 'all',      labelKey: 'all',      statuses: null,                                           icon: LayoutList  },
  { key: 'approved', labelKey: 'approved', statuses: ['head_approved','ready_pickup','borrowed','returned'], icon: CheckCircle },
...
export default function HeadApprovalHistory() {
  ...
  const STATUS_LABEL = useMemo(() => ({
    ...
    ready_pickup:  t('readyForPickup'),
    borrowed:      t('borrowedLabel'),
    returned:      t('returned'),
    rejected:      t('rejected'),
  }), [t]);
  ...
  return (
    <div className="eq-admin" style={{ padding: '24px 28px', minHeight: '100%' }}>
      ...
      <div className="a-titlestrip">
        <div style={{ flex: 1 }}>
          <div className="a-eyebrow">{t('headLabApprovalHistoryEyebrow')}</div>
          ...
          return (
            ...
            >
              <span className="t-label">
                <Icon size={12} /> {t(filter.labelKey)}
              </span>
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          {isError ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--a-mute)' }}>
              ...
              <p style={{ fontFamily: 'var(--serif)', fontSize: 15 }}>{myActedRequests.length === 0 ? t('noReviewsYet') : t('noResultsFound')}</p>
              <p style={{ fontSize: 12, marginTop: 4 }}>{myActedRequests.length === 0 ? t('requestsActionedWillAppear') : t('tryDifferentFilterOrSearch')}</p>
            </div>
          ) : (
                  ...
                  <div key={request.id} style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, width: 140, flexShrink: 0, alignSelf: 'flex-start' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        {isApproved
                          ? <span className="a-pill p-ok"><span className="dot" style={{ background: 'var(--a-ok)' }} />{t('approved')}</span>
