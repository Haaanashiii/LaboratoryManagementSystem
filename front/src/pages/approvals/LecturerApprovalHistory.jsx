...
const STATUS_FILTERS = [
  { key: 'all',       label: 'All',       statuses: null,                                               icon: LayoutList,  pillClass: 'p-mute' },
  { key: 'forwarded', label: 'Forwarded', statuses: ['pending_head','head_approved','ready_pickup','borrowed','returned'], icon: ThumbsUp,    pillClass: 'p-ok'   },
  { key: 'rejected',  label: 'Rejected',  statuses: ['rejected'],                                       icon: XCircle,     pillClass: 'p-bad'  },
  { key: 'returned',  label: 'Returned',  statuses: ['returned'],                                       icon: RotateCcw,   pillClass: 'p-info' },
];
  ...
  return (
    <div className="eq-admin" style={{ minHeight: '100vh' }}>
      ...
      <div className="a-titlestrip">
        <div style={{ flex: 1 }}>
          <div className="a-eyebrow">Lecturer · Approval History</div>
        ...
        <div style={{ padding: '16px 20px 20px' }}>
          {isError ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--a-mute)' }}>
              ...
              <p style={{ fontFamily: 'var(--serif)', fontSize: 15 }}>{requests.length === 0 ? 'No reviews yet' : 'No results found'}</p>
              <p style={{ fontSize: 12, marginTop: 4 }}>{requests.length === 0 ? 'Requests you review will appear here.' : 'Try a different filter or search term.'}</p>
            </div>
          ) : (
                  ...
                  <div key={request.id} style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, width: 140, flexShrink: 0, alignSelf: 'flex-start' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                        {isForwarded
                          ? <span className="a-pill p-ok"><span className="dot" style={{ background: 'var(--a-ok)' }} />Forwarded</span>
