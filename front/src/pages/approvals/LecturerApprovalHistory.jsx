...
import { Search, History, CheckCircle, XCircle, Package } from 'lucide-react';
...
export default function LecturerApprovalHistory() {
  ...
  const forwardedCount = requests.filter(r =>
    ['pending_head', 'head_approved', 'ready_pickup', 'borrowed', 'returned'].includes(r.status)
  ).length;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 py-4 px-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Approval History</h1>
          <p className="mt-0.5 text-sm text-slate-500">A record of all borrow requests you have reviewed.</p>
        </div>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-1.5">
            ...
            <span className="text-xs font-semibold text-emerald-700">{forwardedCount} Forwarded</span>
          </div>
          ...
          {STATUS_FILTERS.map(f => {
            ...
            return (
              <button
                ...
                className={`relative text-[0.8125rem] font-semibold px-4 py-1.5 rounded-[9px] cursor-pointer border-none outline-none transition-colors ${
                  statusFilter === f.key
                    ...
                    <div className="text-center">
                      {requests.length === 0 ? (
                        <>
                          ...
                          <p className="text-xs text-slate-400">Requests you review will appear here.</p>
                        </>
                      ) : (
                        <>
