...
const PENDING_STAGE_LABEL_KEYS = {
...
const STATUS_FILTERS = [
  ...
  { key: 'recently_borrowed', labelKey: 'recentlyBorrowed', statuses: RECENTLY_BORROWED_STATUSES },
];
        ...
        {filteredRequests.length === 0 ? (
          ...
          pagedRequests.map((request) => {
            ...
            const pendingSignalText = hasBlockingActiveRequest
              ? t(PENDING_STAGE_LABEL_KEYS[activeRequest.status] || 'pending')
              : null;
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isRejected && request.rejection_reason && (
                        <div className={`mt-3 rounded-xl px-3 py-2.5 text-xs flex items-start gap-2 ${
                          ...
                          <span><span className="font-semibold">{t('rejectionReason')}: </span>{request.rejection_reason}</span>
                        </div>
                      )}
                      ...
                      {hasBlockingActiveRequest && (
                        <div className={`mt-3 rounded-xl px-3 py-2.5 text-xs flex items-start gap-2 ${
                          ...
                          <span>
                            {t('activeRequestInProgress')}: <span className="font-semibold">{pendingSignalText}</span>. {t('borrowAgainDisabledMsg')}
                          </span>
                        </div>
                      )}
